...
| 17 | SEMANA5-01 | aprobado | A | ninguna | | Soporte de S5 en meses de 29-31 días (regla de duración por mes calendario, sin prorrateo). Especificación cerrada y aprobada por Camilo 25 jul 2026. **Bloqueado por WIP limit (I-09)**: `TICKET-B-GUARDIA-01` sigue `activo` sin DoD verificado — no puede pasar a `activo` hasta que ese ticket cierre o Camilo autorice excepción explícita. |
| 18 | FIX-FALTAPAGAR-MENSUAL-01 | activo | B | FIX-BOLSILLO-MENSUAL-01 | | "Falta por pagar" sumaba el monto completo de bolsillos mensuales (Mercado mensual, Frida, Fondo transporte) en cada semana en vez de una vez al mes. Fix aplicado en `VistaSemanal.tsx` (mismo patrón `idsBolsillosMensuales` de `FIX-BOLSILLO-MENSUAL-01`, aplicado al total agregado). **Excepción de WIP limit (I-09) autorizada explícitamente por Camilo** — abierto pese a `TICKET-B-GUARDIA-01` seguir `activo`. Pendiente: prueba en dev y PR. |
