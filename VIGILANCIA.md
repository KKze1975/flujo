...
## Auditoría — 2026-09-14 (UTC)

**Resumen:** Corrida sobre `vigilancia-auto` sincronizada con `dev` (2 commits nuevos desde la corrida anterior, `9b96c57` y `77db765`, ambos solo documentación — cero cambios de código fuente desde el `4564d05` ya auditado el 07-sep). Se re-verificó por lectura directa una muestra de los hallazgos críticos de corridas previas y siguen presentes, byte-idénticos: sin auth en rutas de escritura no-admin, `CRON_SECRET` fail-open, `next@16.2.6`, `reset-mes` sigue borrando H2 completo; se omiten como pilares aparte por no tener hallazgo nuevo, no repetir no implica que estén resueltos. 1 hallazgo crítico nuevo y 1 hallazgo crítico de proceso, ambos sobre Pilar 1: (1) el propio proyecto abrió y documentó formalmente `tickets/SEC-EXPOSICION-PUBLICA-01.md` — diagnóstico ya completo, confirmando y ampliando con evidencia de `curl` lo que esta rutina venía señalando desde el 24-ago (ahora incluye lectura anónima de las páginas SSR y de casi todas las rutas `GET`, no solo las de escritura) — sigue `estado: propuesto`, sin elegir opción, sin construir nada; (2) `ESTADO.md` (entrada del 11 sept 2026) documenta que esta misma rutina de vigilancia citó el valor real del PIN de administrador en texto plano dentro de sus propias corridas del 24-ago y 31-ago (`8e13d90`, `44341d5`) — la rutina que debía señalar la fuga la repitió dos veces. Esta corrida no cita el valor del PIN en ningún punto, precisamente por ese hallazgo.

### Pilar 1 — Seguridad

1. **CRÍTICO, confirmado con verificación de código independiente — exposición pública sin autenticación, ahora formalizada como `tickets/SEC-EXPOSICION-PUBLICA-01.md` (Tier B, `estado: propuesto`, `halt_criterio: 7`).** El ticket documenta con `curl` (11 sept 2026, evidencia en `ESTADO.md:7089-7108`) que además de las rutas de escritura ya señaladas por esta rutina desde el 24-ago, las páginas SSR y las rutas `GET` también sirven datos financieros reales sin ninguna sesión: `/` (200, HTML con `getMeses()`), `/mes/[mes]` (200, 104 KB), `/api/conceptos` (200, 63 conceptos reales con monto y nombre), entre otras 13 rutas más en la tabla de `ESTADO.md:7121-7150`. Verificado independientemente en esta corrida, por lectura de código (no del ticket): `app/page.tsx:7-11` llama `getProvider().getMeses()` en un Server Component sin ningún chequeo de sesión antes de renderizar; `app/api/consumos/[id]/route.ts:5-17` (`DELETE`) y `app/api/mes/[mes]/movimientos/[id]/route.ts` (handler `PATCH` completo) no importan ni llaman `isAdminRequestAuthorized` ni ningún otro mecanismo de identidad; `app/api/ingresos/camilo/[mes]/route.ts` igual. No existe `middleware.ts` ni `proxy.ts` en la raíz del repo (`find` sin resultados) — confirmado, no hay ninguna capa transversal que pudiera cerrar esto de un solo cambio. El ticket sigue `propuesto`: ninguna de las opciones (A) protección de despliegue Vercel, (B) auth propia vía `proxy.ts`, (C) combinación, ni la mitigación inmediata (D) se ha aplicado — el hueco reportado en corridas anteriores de esta misma rutina (24-ago, hallazgo Pilar 1 #2; 31-ago; 07-sep, mencionado en el resumen) sigue exactamente igual de abierto, ahora con alcance documentado más amplio que solo escritura. OWASP A01:2021 Broken Access Control, agravado por A05:2021 (Security Misconfiguration — Vercel Deployment Protection en `enabled: false` en los tres métodos, según `ESTADO.md:7080-7081`, dato de plataforma no verificable con las herramientas de esta rutina).

2. **CRÍTICO, hallazgo de proceso sobre esta misma rutina — fuga del PIN real repetida dentro de `VIGILANCIA.md`.** `ESTADO.md` (entrada "Cierre de sesión — verificación adicional del PIN filtrado...", 11 sept 2026, líneas 7257-7280) documenta que el valor real del PIN de administrador, ya filtrado en texto plano en el commit `9367235` de `ESTADO.md`, fue **citado textualmente otra vez** dentro de los reportes generados por esta rutina de vigilancia semanal, en los commits `8e13d90` (corrida 2026-08-24) y `44341d5` (corrida 2026-08-31) — el hallazgo de seguridad que debía señalar la fuga la repitió, sin que nadie lo notara hasta el 11 de septiembre. Esas dos entradas de este mismo archivo ya están publicadas en el historial de `vigilancia-auto` (rama remota) y no se reescriben (regla append-only de este mismo archivo) — el valor sigue recuperable por `git log -p` sobre esos dos commits, independientemente de la rotación futura del PIN. Esta entrada no repite el valor. Corresponde al candidato a invariante ya registrado en `ESTADO.md:7274-7280` ("no citar textualmente un secreto ya identificado como filtrado dentro de un nuevo artefacto"), pendiente de aprobación de Camilo — mismo patrón I-04/I-08 (dato sensible commiteado), ahora sobre una credencial repetida por la propia herramienta de auditoría.
