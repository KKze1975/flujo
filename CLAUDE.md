...
**Cuando un ticket queda `paso_actual: "... pendiente de Tester"` (o `rol_activo: coder` con la construcción ya reportada como terminada), la sesión que está hablando con Camilo sobre ese ticket — Manager en sentido amplio, exista o no como invocación formal de `manager.md` — despacha Tester como llamada de subagente aparte (`Agent` tool, briefing: solo el diff final + el DoD del ticket, nunca el razonamiento propio ni el reporte del Coder). Nunca lo verifica inline en la misma conversación, aunque tenga las herramientas para hacerlo.** Corrección explícita de Camilo, 16 ago 2026, tras un incidente real (`PANEL-LOG-EVENTOS-01`) — detalle completo y las 3 decisiones de diseño resueltas en `ARQUITECTURA_MULTIAGENTE.md` §12.15 del vault. Verificable con `node scripts/check-ticket.mjs --audit-tester-gate`: detecta tickets "pendiente de Tester" donde `rol_activo` nunca pasó a `tester` — reusa el campo que `tester.md` ya escribe (§12.12), no es un mecanismo nuevo.
